...
const Header = () => {
  ...
  return (
    // REPLACED: Header với style glassmorphism, sticky và hiệu ứng blur
    <header className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-lg">
      <div className="container flex h-16 items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          {/* REPLACED: Desktop Logo & Navigation với style mới */}
          <div className="hidden items-center gap-8 md:flex">
            ...
            <nav className="flex items-center gap-6 text-sm font-medium">
              {navLinks.map((link) => (
                <Link key={link.to} to={link.to} className="text-foreground/80 hover:text-primary transition-colors font-semibold">
                  {link.label}
                </Link>
              ))}
            </nav>
        ...
        <div className="flex items-center gap-3">
          {isAuthenticated ? (
            ...
            <div className="hidden items-center gap-2 md:flex">
              <Button asChild variant="ghost"><Link to="/login">Đăng nhập</Link></Button>
              <Button asChild className="bg-primary text-primary-foreground hover:bg-primary/90"><Link to="/register">Đăng ký</Link></Button>
            </div>
          )}
        </div>
      </div>
